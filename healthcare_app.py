...
model_cancer = pickle.load(open('breat_cancer.pkl', 'rb'))
model_malaria = load_model('malariadisease.h5')
model_pneumonia = load_model('pneumonia_disease1.h5')
...
def heartdisease():
    if request.method == 'POST':
        ...
        if prediction==1:
            return render_template('heartdisease.html', prediction_text="The person seems to have Heart Disease.", title='Heart Disease')
        else:
            return render_template('heartdisease.html', prediction_text="The person does not have any Heart Disease.", title='Heart Disease')
    else:
...
def liverdisease():
    if request.method == 'POST':
        ...
        if prediction==1:
            return render_template('liverdisease.html', prediction_text=" The person seems to have Liver Disease.", title='Liver Disease')
        else:
            return render_template('liverdisease.html', prediction_text=" The person does not have any Liver Disease.", title='Liver Disease')
    else:
...
def breastcancer():
    if request.method == 'POST':
        ...
        if prediction==1:
            return render_template('cancer.html', prediction_text="The tumor is malignant.", title='Breast Cancer')
        else:
            return render_template('cancer.html', prediction_text=" The tumor is benign.", title='Breast Cancer')
    else:
...
def malariadisease():
    if request.method=="GET":
        ...
        if prediction[0][0]>=0.5:
            return render_template('malariadisease.html', prediction_text=" The cell image indicates the presence of Malaria.", image_name = f.filename, title='Malaria Disease')
        else:
            return render_template('malariadisease.html', prediction_text=" The person does not seem to have Malaria.", image_name= f.filename, title='Malaria Disease')
...
def pneumoniadisease():
    if request.method=="GET":
        ...
        if pred[0]==1:
            return render_template('pneumoniadisease.html', prediction_text="This Chest X-Ray shows an area of lung inflammation indicating the presence of Pneumonia.", file_name = f.filename, title='Pneumonia Disease')
        else:
            return render_template('pneumoniadisease.html', prediction_text="The person does not seem to have Pneumonia.", file_name= f.filename, title='Pneumonia Disease')
